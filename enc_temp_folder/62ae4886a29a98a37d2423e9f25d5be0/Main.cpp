...
struct CKgCamera
{
	CKgVector3D m_camera_pos;
	CKgVector3D forward_vector;
	CKgVector3D up_vector;
	double** ViewMatrix;

	CKgCamera(const CKgVector3D& m_camera_pos, const CKgVector3D& forward_vector, const CKgVector3D& up_vector)
		: m_camera_pos(m_camera_pos),
	...
	~CKgCamera(){ free_dmatrix(ViewMatrix, 4, 4); }

	void MoveBy(double OffsetX, double OffsetY, double OffsetZ)
	{
		m_camera_pos.x += OffsetX;
		m_camera_pos.y += OffsetY;
		m_camera_pos.z += OffsetZ;
		ViewMatrix = ComputeViewMatrix();
	...
	{
		CKgVector3D Forward = forward_vector;
		CKgVector3D CameraUp = up_vector;
		CKgVector3D Camera = m_camera_pos;
		Forward.Normalize();
		CameraUp.Normalize();
...
public:
	...
	CKhuGle3DSprite(int nW, int nH, double Fov, double Far, double Near, KgColor24 fgColor, CKgCamera *p_camera);
	~CKhuGle3DSprite();
...
CKhuGle3DSprite::CKhuGle3DSprite(int nW, int nH, double Fov, double Far, double Near, KgColor24 fgColor, CKgCamera *p_camera) {
	m_fgColor = fgColor;
	m_Camera = p_camera;
...
void CKhuGle3DSprite::Render()
{
	...
	double** ViewMatrix = m_Camera->ViewMatrix;
	...
	for(auto &Triangle: SurfaceMesh)
	{
		...
		if(Normal.Dot(Triangle.v0 - m_Camera->forward_vector) < 0.)
		{
...
CThreeDim::CThreeDim(int nW, int nH) : CKhuGleWin(nW, nH) 
{
	...
	m_camera = new CKgCamera(CKgVector3D(0., -0.2, -2), CKgVector3D(0, 0.2, 2), CKgVector3D(0., 1., 0.));

	m_pObject3D = new CKhuGle3DSprite(m_pGameLayer->m_nW, m_pGameLayer->m_nH, Pi / 2., 1000., 0.1, KG_COLOR_24_RGB(255, 0, 255), m_camera);
...
void CThreeDim::Update()
{
	...
	/*
	double NewX = m_Camera->x * cos(Pi / 1000.) - m_Camera->z * sin(Pi / 1000.);
	double NewZ = m_Camera->x * sin(Pi / 1000.) + m_Camera->z * cos(Pi / 1000.);
	m_Camera->x = NewX;
	m_Camera->z = NewZ;
	*/
	m_pScene->Render();
